...
Dim sText As String
   ...
   If (m_iSearchCol > 0) And (m_iSearchCol < m_iCols) Then
      If Not bMatchCase Then sSearchString = UCase$(sSearchString)
      If (m_iSelRow = 0) Then
         If (bLoop) Then
            iStart = m_iSelRow + 1
         Else
      ...
      For iRow = iStart To m_iRows
         If (m_tRows(iRow).bVisible) Or Not (bVisibleRowsOnly) Then
            If Not IsMissing(m_tCells(m_iSearchCol, iRow).sText) Then
               If bMatchCase Then
                  sText = m_tCells(m_iSearchCol, iRow).sText
               Else
...
Public Sub MoveColumn(lColumn As Long, lOrder As Long)
   Dim iCol             As Long
...
Private Sub m_cHeader_ColumnEndDrag(lColumn As Long, lOrder As Long)
   ...
   Call MoveColumn(lColumn, lOrder)
